...
export function useFhevm(parameters: {
  ...
  const refresh = useCallback(() => {
    console.log(`[MediRecX useFhevm] 🔄 刷新FHEVM连接...`);
  ...
  useEffect(() => {
    ...
    if (_isRunning === false) {
      // cancelled
      console.log("[MediRecX useFhevm] ⏸️  FHEVM连接已暂停");
      if (_abortControllerRef.current) {
        _abortControllerRef.current.abort();
        _abortControllerRef.current = null;
      }
    ...
    if (_isRunning === true) {
      ...
      console.log(`[MediRecX useFhevm] 🚀 开始创建FHEVM实例...`);
      ...
      createFhevmInstance({
        ...
        onStatusChange: (s) =>
          console.log(`[MediRecX useFhevm] 状态变更: ${s}`),
      })
        .then((i) => {
          console.log(`[MediRecX useFhevm] ✅ FHEVM实例创建成功！`);
        ...
        .catch((e) => {
          console.log(`[MediRecX useFhevm] ❌ FHEVM实例创建失败: ` + e.name);
          if (thisSignal.aborted) return;
