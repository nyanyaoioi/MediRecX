...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    console.log("[MediRecX RelayerSDKLoader] 开始加载FHEVM SDK...");
    // Ensure this only runs in the browser
    if (typeof window === "undefined") {
      console.log("[MediRecX RelayerSDKLoader] window === undefined");
    ...
    if ("relayerSDK" in window) {
      if (!isFhevmRelayerSDKType(window.relayerSDK, this._trace)) {
        console.log("[MediRecX RelayerSDKLoader] window.relayerSDK === undefined");
        throw new Error("RelayerSDKLoader: Unable to load FHEVM Relayer SDK");
      }
      console.log("[MediRecX RelayerSDKLoader] SDK已存在，直接使用");
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        if (!isFhevmWindowType(window, this._trace)) {
          console.log("[MediRecX RelayerSDKLoader] script onload FAILED...");
          reject(
            new Error(
        ...
        console.log("[MediRecX RelayerSDKLoader] ✅ SDK加载成功！");
        resolve();
      };

      script.onerror = () => {
        console.log("[MediRecX RelayerSDKLoader] script onerror... ");
        reject(
          new Error(
      ...
      console.log("[MediRecX RelayerSDKLoader] 添加SDK脚本到DOM...");
      document.head.appendChild(script);
      console.log("[MediRecX RelayerSDKLoader] 脚本已添加，等待加载...")
    });
  }
}
